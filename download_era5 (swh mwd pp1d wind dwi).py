...
def process_grib_file_df(file_path, target_latitude, target_longitude):
    """
    ...
    for grb in grbs:
        try:
            ...
            dist = np.sqrt((lats - target_latitude)**2 + (lons - target_longitude)**2)
            if np.any(dist < 1e-6):
...
def main():
    ...
    while True:
        try:
            target_longitude = float(input("Enter target longitude (e.g., -9.581666670): "))
            if not (-180 <= target_longitude <= 180):
                print("Longitude must be between -180 and 180. Please try again.")
                continue
            break
        except ValueError:
            print("Invalid input for longitude. Please enter a number.")

    while True:
        try:
            target_latitude = float(input("Enter target latitude (e.g., 41.14833299): "))
            if not (-90 <= target_latitude <= 90):
                print("Latitude must be between -90 and 90. Please try again.")
                continue
            break
        except ValueError:
            print("Invalid input for latitude. Please enter a number.")

    # Log the target longitude and latitude
    logging.info(f"Target Longitude: {target_longitude}, Target Latitude: {target_latitude}")

    # Define the bounding box (degrees) for the target area based on user input.
    # Format: [North, West, South, East] as required by the MARS request.
    BUFFER = 0.25
    NORTH = target_latitude + BUFFER
    SOUTH = target_latitude - BUFFER
    EAST = target_longitude + BUFFER
    WEST = target_longitude - BUFFER
    ...
    if user_option == '2':
        ...
        with ProcessPoolExecutor(max_workers=os.cpu_count()) as executor:
            futures = {executor.submit(process_grib_file_df, file, target_latitude, target_longitude): file for file in grib_files}
        ...
        with ProcessPoolExecutor(max_workers=os.cpu_count()) as executor:
            ...
            for year in YEARS:
                for month in range(1, 13):
                    ...
                    if file_path:
                        future = executor.submit(process_grib_file_df, file_path, target_latitude, target_longitude)
